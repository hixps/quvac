...
def quvac_evaluation(params):
    ...
    for param_key,param in params.items():
        ...
        ini_data['fields'][category][key] = param * scales.get('key', 1)
...
def run_optimization(ax_client, executor, n_trials, max_parallel_jobs,
                     experiment_file):
    jobs = []
    submitted_jobs = 0
...
def parse_args():
    ...
    argparser.add_argument("--optimization", default=None,
                           help="Yaml file with optimization parameters")
...
def cluster_optimization(ini_file, optimization_file, save_path=None, wisdom_file=None):
    ...
    err_msg = f'Only field parameters could be optimized but you have {optimization_keys}'
    assert sum([key.startswith('field') 
                for key in optimization_keys]) == len(optimization_keys), err_msg
    ...
    ax_client.create_experiment(
        ...
        outcome_constraints=optimization_params.get('outcome_constraints', None),  # Optional.
    )
    ...
    run_optimization(ax_client, executor, n_trials, max_parallel_jobs,
                     experiment_file)
