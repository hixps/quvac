...
def get_grid_params(grid_xyz, grid_t):
    ...
    grid_print = grid_str.format(grid_xyz_size, x_box, y_box, z_box, grid_t_size, t_box)
    return grid_print
...
def get_performance_stats(perf_stats):
    ...
    perf_print = performance_str.format(
        timings["field_setup"],
        timings["vacem_setup"],
        timings["amplitudes"],
...
def get_parallel_performance_stats(perf_stats):
    ...
    timings = {k: format_time(t) for k, t in timings.items()}
    memory = {k: format_memory(m) for k, m in perf_stats["memory"].items()}
    perf_print = performance_parallel_str.format(
        timings["jobs"],
        timings["postprocess"],
        timings["total"],
        memory["maxrss_jobs"],
